...
if ! "$PY" -c "from transformers import Sam3Model  # noqa: F401" 2>/dev/null; then
    echo "ERROR: SAM3 requires transformers ≥5.5 in $VENV_DIR (Sam3Model missing)." >&2
    echo "  Fix: $PY -m pip install -U 'transformers[torch]>=5.5'" >&2
    exit 1
fi
...
PYTHONPATH="$ROOT_DIR:$ROOT_DIR/autocalib-api" \
    "$UVICORN" app.main:app --host 0.0.0.0 --port 8000 --reload \
    --reload-dir "$ROOT_DIR/autocalib-api" \
    --reload-dir "$ROOT_DIR/autoabsmap" \
    --reload-dir "$ROOT_DIR/calib_gen" \
...
cd "$ROOT_DIR/autocalib-frontend"
