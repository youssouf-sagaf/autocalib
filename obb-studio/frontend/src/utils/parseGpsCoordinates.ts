 ...
 * Supports decimal pairs (lat,lng or lng,lat), Mapbox/Google URLs, and WKT POINT.
 */
...
function pairToLngLat(a: number, b: number): { lng: number; lat: number } | null {
  ...
  const asLatLng = a >= 35 && a <= 72 && b >= -15 && b <= 20;
  const asLngLat = b >= 35 && b <= 72 && a >= -15 && a <= 20;
  if (asLatLng && !asLngLat) return { lng: b, lat: a };
  if (asLngLat && !asLatLng) return { lng: a, lat: b };
  return { lng: b, lat: a };
}

function toResult(lng: number, lat: number): ParsedGpsCoordinates {
  return { lng, lat, label: `${lat.toFixed(6)}, ${lng.toFixed(6)}` };
}
...
export function parseGpsCoordinates(input: string): ParsedGpsCoordinates | null {
  ...
  if (atMatch?.[1] != null && atMatch[2] != null) {
    const hit = tryPair(parseFloat(atMatch[1]), parseFloat(atMatch[2]));
    if (hit) return hit;
  }
  ...
  if (latParam?.[1] != null && lngParam?.[1] != null) {
    ...
    if (isValidLat(lat) && isValidLng(lng)) return toResult(lng, lat);
  }

  const qMatch = trimmed.match(/[?&]q=(-?\d+(?:\.\d+)?)[,%20\s]+(-?\d+(?:\.\d+)?)/i);
  if (qMatch?.[1] != null && qMatch[2] != null) {
    const hit = tryPair(parseFloat(qMatch[1]), parseFloat(qMatch[2]));
    if (hit) return hit;
  }
