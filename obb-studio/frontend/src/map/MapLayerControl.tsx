...
function isInsideMetropolitanFR(lng: number, lat: number): boolean {
  return (
    lng >= PLEIADES_FR_BBOX.west &&
    lng <= PLEIADES_FR_BBOX.east &&
    lat >= PLEIADES_FR_BBOX.south &&
    lat <= PLEIADES_FR_BBOX.north &&
    lat >= PLEIADES_FR_BBOX.south
  );
}
...
export function MapLayerControl() {
  ...
  const active = useAppSelector((s) => s.studio.imagerySource);
  const view = useAppSelector((s) => s.studio.mapViewState);
  ...
  const insideFR = view
    ? isInsideMetropolitanFR(view.longitude, view.latitude)
    : true;
      ...
      {open && (
        <div ref={popoverRef} className={styles.popover} role="menu">
          ...
          {IMAGERY_SOURCES.map((src) => {
            const disabled = src === 'ign-pleiades-2026' && !insideFR;
            ...
            return (
              <button
                ...
                title={disabled ? t('imagerySource.hintFranceOnly') : undefined}
              >
