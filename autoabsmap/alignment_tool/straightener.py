...
def _rebuild_geoslot(local: _LocalSlot, ref_lng: float, ref_lat: float) -> GeoSlot:
    ...
    return GeoSlot(
        ...
        slot_type=local.geo_slot.slot_type,
    )
...
def _collect_row_between_anchors(
    ...
    angle_tol: float,
    pad_along: float,
    manual_angle_tol: float,
    manual_corridor_hw: float,
) -> list[_LocalSlot]:
    """Slots on the row segment between anchor A and B (small endpoint slack only)."""
    ...
    def in_corridor_band(s: _LocalSlot, half_width: float) -> bool:
        dx = s.cx - mx
        dy = s.cy - my
    ...
    for s in all_locals:
        if s.geo_slot.slot_id in by_id:
            continue
        is_manual = s.geo_slot.source == SlotSource.manual
        tol = manual_angle_tol if is_manual else angle_tol
        hw = manual_corridor_hw if is_manual else corridor_hw
        if not _slot_axis_aligns_with_row(s.angle_rad, row_angle, tol):
            continue
        if not in_corridor_band(s, hw):
            continue
...
class RowStraightener:
    ...
    def straighten(
        ...
        manual_angle_tol = math.radians(self._s.manual_angle_tolerance_deg)
        avg_w = (local_a.width + local_b.width) / 2
        anchor_w = max(local_a.width, local_b.width)
        corridor_hw = self._s.corridor_width_factor * anchor_w
        manual_corridor_hw = self._s.manual_corridor_width_factor * anchor_w
        pad_along = self._s.endpoint_pad_width_factor * avg_w

        row = _collect_row_between_anchors(
            local_a, local_b, all_local, row_angle,
            corridor_hw, angle_tol, pad_along,
            manual_angle_tol, manual_corridor_hw,
        )
