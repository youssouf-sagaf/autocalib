...
class MapboxImageryProvider:
    ...
    def fetch_geotiff(
        ...
        img_w, img_h = self._image_size_for_bbox(
            ...
            soft_cap_m=self._settings.soft_cap_m,
        )
    ...
    def _image_size_for_bbox(
        ...
        When the ROI exceeds the 1280 px Mapbox cap, GSD degrades. We log at
        ``info`` level for ROIs within ``soft_cap_m`` (tolerated degradation)
        and at ``warning`` beyond it (problematic — should be auto-tiled).
        """
        wpx = _world_px(zoom)
        ...
        if ground_max <= soft_cap_m:
            logger.info(
                "ROI %.0f×%.0fm clamped to %dx%d px at zoom %.2f "
                "(effective GSD ≈ %.3f m/px, within %.0fm soft cap).",
                ground_w, ground_h, img_w, img_h, zoom, effective_gsd, soft_cap_m,
            )
        else:
            logger.warning(
                "ROI %.0f×%.0fm exceeds %.0fm soft cap at zoom %.2f: "
                "need %dx%d px, clamped to %dx%d (effective GSD ≈ %.3f m/px). "
                "Enable auto-tiling for this ROI.",
                ground_w, ground_h, soft_cap_m, zoom,
                needed_w, needed_h, img_w, img_h, effective_gsd,
            )
        return img_w, img_h
