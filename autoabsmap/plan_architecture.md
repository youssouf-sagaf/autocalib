# `autoabsmap` — package architecture

**Monorepo index:** [`../plan_architecture.md`](../plan_architecture.md) — cross-package constraints, `slot_id`, full dependency graph.
...
```mermaid
flowchart TD
    subgraph poc [POC — autocalib-frontend + autocalib-api]
        UI_POC["Absmap FE (React + IMapProvider)"]
        API_POC["autocalib-api (FastAPI standalone)"]
...
| 3 | `ml/detection` | Identical SAM3 vehicle OBB outputs (same checkpoint) |
...
tests/golden/
  case_001/
    ...
    detections_raw.json           # R&D detections before GeometricEngine
...
autocalib/autoabsmap/
  ...
  config/
    __init__.py
    settings.py              # Pydantic BaseSettings: SAM3, imagery, geometry, fusion, pipeline
  ...
  imagery/
    ...
    ign.py                   # IGNImageryProvider — WMTS presets (ortho, Pleiades 2026)
    google.py                # GoogleMapsImageryProvider — satellite tiles for jobs

  ml/
    __init__.py
    protocols.py             # Detector protocol (injectable / testable)
    models.py                # DetectionResult (Pydantic)
    sam3_detector.py         # Sam3VehicleDetector implements Detector (mask-fit OBBs)
    detection.py             # re-exports Sam3VehicleDetector
  ...
  generator_engine/          # ★ Block 3 — Autoabsmap Generator Engine (High Brain)
    __init__.py
    models.py                # PipelineRequest, PipelineResult, StageProgress, PixelSlot, HintMasks, SlotSource
    prior.py                 # GeometricPrior from SAM3 + mask PCA
    runner.py                # ParkingSlotPipeline(imagery, segmenter, detector).run(request)
    stages.py                # fetch_imagery, segment, detect (SAM3), geometric_engine, export
    geometric_engine.py      # GeometricEngine — row extension, gap fill, prior + hints in dedup
    stage_artifacts.py       # Debug dumps: 04_evidence/, 05_export/, …
  ...
  alignment_tool/            # ★ Block 7 — Alignment "mise au carré" Automation Tool
    ...
    straightener.py          # RowStraightener.straighten(anchor_a, anchor_b, all_slots) → corrected GeoSlot[]
                             # No engine-local models.py: slots are export.models.GeoSlot; POST body = StraightenRequest in autocalib-api
...
**Geometric prior (generator_engine):** `build_geometric_prior_from_detection_and_mask` (in `prior.py`) computes a frozen `GeometricPrior` from SAM3 vehicle OBB medians (width / height / width-axis angle) and the seg ∩ ROI mask (PCA orientation + coverage confidence). Selection policy: strong SAM3 → mask PCA → `GeometrySettings` defaults. `baseline_slots` are raw SAM3 anchors for the learning loop. Legacy stripe **marking_seed** / typed **evidence/** modules were removed.
...
class StageProgress(BaseModel):
    ...
    # The MultiCropOrchestrator in autocalib-api wraps this into an OrchestratorProgress
...
**`JobRequest` / `JobResult` — multi-crop, owned by `autocalib-api`:**
...
class OrchestratorProgress(BaseModel):
    # autocalib-api wrapper — adds crop context around the pure StageProgress from autoabsmap
...
```python
# autocalib-api / services / pipeline_service.py
...
provider = build_imagery_provider(settings)  # MapboxImageryProvider, IGNImageryProvider,
                                             # GoogleMapsImageryProvider — same Protocol
...
## Imagery strategy & CRS (foundation)
...
autoabsmap/imagery/
  ...
  ign.py                 # IGNImageryProvider    — WMTS tiles (ortho, Pleiades 2026)
  google.py              # GoogleMapsImageryProvider — satellite for operator jobs
```
...
The session is stored per-job, with separate layers for each stage of the pipeline. This enables per-layer CV analysis (SegFormer signals vs SAM3 detection signals are distinct).
...
sessions/{session_id}/
  ...
  per_crop/{crop_index}/
    ...
    detection_raw.geojson             # SAM3 vehicle OBBs before GeometricEngine
  ...
  pending_sync/{sync_id}.json         # written by autocalib-api on client slot sync when a client is set;
                                      # consumed by async B2B worker (final_slots, crops, removed_prod_slots)
```
...
|---|---|
| `sam3` | SAM3 vehicle detection (mask-fit OBB anchor) |
...
The stored layers feed two separate CV improvement paths. They must be kept distinct — the learning signal for SegFormer is not the same as for SAM3 detection.

**Offline tooling (not product HTTP):** `learning_loop/benchmark.py` and `learning_loop/dataset_builder.py` are CLI / batch utilities for retraining evaluation and export layout — kept intentionally, not dead code.
...
| `source` attribution | Error localization by generation path (e.g. `row_extension` failing more than `sam3` → geometry bug, not model bug) |
...
class BenchmarkReport(BaseModel):
    candidate_bundle: str                # e.g. "segformer-v3 + sam3-v1"
    sessions_tested: int
...
Transforms captured sessions into training-ready datasets. Separates SegFormer signals from SAM3 detection signals because their learning paths are distinct.
...
## Related — operator stack (`autocalib-api`, calib_gen, pairing)

The **absolute map** Python package (`autoabsmap/`) stays focused on geo slot generation and learning-loop capture for that domain.

**HTTP orchestration** for operators lives in **[`autocalib-api`](../autocalib-api/plan_architecture.md)** (successor to `autoabsmap-api` / `operator-api`): one FastAPI service over **`autoabsmap`** (`/api/v1/jobs`, `/api/v1/clients/…/slots/save`) and **namespaced** routes for **`calib_gen`** and **`pairing`** ([`calib_gen/plan_architecture.md`](../calib_gen/plan_architecture.md), [`pairing/plan_architecture.md`](../pairing/plan_architecture.md)), without importing those packages into **`autoabsmap`**. Learning-loop trace is a **sidecar** on `slots/save` when `job_id` + `edit_events` are sent; **B2B geography write** is **synchronous** in the same request — see [`integration.md`](../integration.md) §7.

**Operator UI** remains **[`autocalib-frontend`](../autocalib-frontend/plan_architecture.md)** with workspace modes (absolute map, calib editor, pairing) and a single `VITE_API_URL` pointing at `autocalib-api`.
