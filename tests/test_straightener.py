...
def test_straighten_includes_pipeline_synthetic_sources_on_segment():
    """gap_fill / row_extension on the A→B segment are aligned like sam3."""
    pitch, slot_w, slot_h = 2.8, 2.5, 5.0
    sources = [
        SlotSource.sam3,
        SlotSource.gap_fill,
        SlotSource.row_extension,
        SlotSource.sam3,
    ]
    row_slots = [
        _make_slot(f"row-{i}", cx=i * pitch, cy=0.0, w=slot_w, h=slot_h, angle=0.0, source=src)
        for i, src in enumerate(sources)
    ]

    corrected = RowStraightener().straighten("row-0", "row-3", row_slots)
    assert {s.slot_id for s in corrected} == {"row-0", "row-1", "row-2", "row-3"}


def test_straighten_includes_manual_slots_with_loose_angle():
    """Manual extend/clone slots use a wider angle gate but stay on the segment."""
    pitch, slot_w, slot_h = 2.8, 2.5, 5.0
    row_slots = [
        _make_slot("row-0", cx=0.0, cy=0.0, w=slot_w, h=slot_h, angle=0.0),
        _make_slot(
            "row-1", cx=pitch, cy=0.15, w=slot_w, h=slot_h,
            angle=math.radians(55), source=SlotSource.manual,
        ),
        _make_slot("row-2", cx=2 * pitch, cy=0.0, w=slot_w, h=slot_h, angle=0.0),
    ]

    corrected = RowStraightener().straighten("row-0", "row-2", row_slots)
    assert {s.slot_id for s in corrected} == {"row-0", "row-1", "row-2"}
