...
def _tile_crop(
    ...
    *,
    ...
    When *auto_tile* is False, always returns a single tile with the original
    polygon — the imagery provider may clamp resolution for very large ROIs
    (warning is logged downstream).
    """
    ...
    if not auto_tile:
        logger.warning(
            "Auto-tiling disabled but ROI is %.0f×%.0fm > %.0fm tile budget. "
            "Imagery provider may clamp resolution (effective GSD will degrade).",
            ground_w, ground_h, tile_ground,
        )
        return [_TiledCrop(fetch_window=None, roi=crop.polygon, hints=crop.hints)]
    ...
    async def run(
        ...
        *,
        ...
        if settings.debug_artifacts:
            ...
            artifacts_base = pkg_root / "artifacts" / f"{stamp}_{job_id[:8]}"

        auto_tile = bool(getattr(settings.imagery, "auto_tile", True))
        all_tiles: list[_TiledCrop] = []
        for crop in crops:
            all_tiles.extend(_tile_crop(crop, target_gsd, auto_tile=auto_tile))
