...
from app.services.pipeline_service import build_orchestrator, get_imagery_provider
...
async def _run_job(job_id: str, request: JobRequest) -> None:
    ...
    try:
        ...
        imagery_override = (
            get_imagery_provider(request.imagery_source)
            if request.imagery_source
            else None
        )
        result = await orchestrator.run(
            request.crops, job_id, on_progress,
...
async def submit_job(request: JobRequest, background_tasks: BackgroundTasks) -> PipelineJob:
    ...
    await job_store.remember_crop_polygons(
        job_id,
        [crop.polygon for crop in request.crops],
    )
...
async def stream_progress(job_id: str) -> EventSourceResponse:
    ...
    # Late subscribers (reconnect, slow network, second tab): the `done` sentinel was
    # already broadcast to queues that may have been empty — replay terminal state
    # instead of blocking forever on queue.get().
    if job.status == JobStatus.done:

        async def replay_done() -> None:
            yield {"event": "done", "data": "{}"}

        return EventSourceResponse(replay_done())

    if job.status == JobStatus.failed:

        async def replay_failed() -> None:
            yield {"event": "failed", "data": "{}"}

        return EventSourceResponse(replay_failed())
    ...
    async def event_generator():
        try:
            # Subscribe may race with runner completion — re-check status after queue wiring.
