---
name: autoabsmap-parity-test
description: >-
  Compare autoabsmap rewrite outputs against R&D golden files module by module.
...
Validate that the `autoabsmap` rewrite produces equivalent results to the R&D pipeline (`absolutemap-gen`) by comparing outputs at each layer.

## Prerequisites

Golden files must exist in `tests/golden/case_NNN/` with R&D outputs captured before the rewrite. See `autoabsmap_architecture.md` section "Parity validation."
...
from autoabsmap.geometry.engine import GeometricEngine
