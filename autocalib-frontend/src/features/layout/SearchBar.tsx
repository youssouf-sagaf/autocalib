...
import { normalizeLocale } from '../../i18n/LanguageToggle';
...
interface Suggestion {
  ...
  /** Value shown in the input after selection (e.g. ``48.853400, 2.348800``). */
  inputLabel?: string;
}
...
export function SearchBar({ onFlyTo }: SearchBarProps) {
  ...
  const geocode = useCallback(
    async (text: string) => {
      ...
      try {
        const url = `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(text)}.json?access_token=${MAPBOX_TOKEN}&limit=5&language=${lang}`;
